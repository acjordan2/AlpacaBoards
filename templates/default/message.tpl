...
<body class="regular">
  <div class="body">
    ...
    <h1>Message Detail</h1><b>Board:</b> {$board_title}<br />
    <b>Topic:</b> 
...
//<![CDATA[
    ...
    <form method="get" action="/postmsg.php" style="display:inline;">
      ...
      "topic" value="{$topic_id}" /><input type="hidden" name="board" value=
      "{$board_id}" /><input type="submit" value="Edit this message" />
    </form>
	...
	{foreach from=$revision_history item='table'}
		#{$table.revision_no + 1} {if $revision_no == $table.revision_no}<b>{else}<a href="/message.php?id={$message_id}&amp;topic={$topic_id}&amp;r={$table.revision_no}">{/if}: 
