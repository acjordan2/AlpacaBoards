{include file="header.tpl"}
	...
	<br />
	<br />
	<b>Added by:</b> <a href="{$base_url}/profile.php?user={$link_user_id}">{$link_username}</a><br />
	<b>Date:</b> {$created|date_format:$dateformat}<br />
	<b>Code:</b> <a href="{$base_url}/linkme.php?l=SS{$link_id}">{$short_code}</a><br />
 	<b>Hits:</b> {$hits}<br />
	...
	<b>Share:</b> <a href="{$base_url}/ss.php?l={$short_code}">{$domain}/ss.php?l={$short_code}</a><br /><br />
	<b>Categories:</b> {foreach from=$tags key=header item=tag}{$tag.title}, {/foreach}
	<form action="{$base_url}/linkme.php?l={$link_id}" method="POST" id="link_fav">
		<b>Options:</b>
...
{/if}
	...
	<div class="userbar">
		...
		|</span> 
		<a href="{$base_url}/postmsg.php?link={$link_id}">Post New Message</a> |
		<a href="https://wiki.endoftheinter.net/index.php/Help:Rules">Help</a>
	</div>
	<div class="infobar" id="u0_2">
        {if $current_page > 1} <span><a href="{$base_url}/linkme.php?l={$link_id}&amp;page=1">First Page</a> |</span>{/if}
        {if $current_page > 2}<span><a href="{$base_url}/linkme.php?l={$link_id}&amp;page={$current_page - 1}">Prev Page</a> |</span>{/if}
        Page {$current_page} of <span>{$page_count}</span> 
        {if $current_page < $page_count - 1}<span>| <a href="{$base_url}/linkme.php?l={$link_id}&amp;page={$current_page + 1}">Next Page</a></span> {/if}
        {if $current_page < $page_count}<span>| <a href="{$base_url}/linkme.php?l={$link_id}&amp;page={$page_count}">Last Page</a></span>{/if}
    </div>
{$i = 5}
{foreach from=$messages key=header item=table}
	<div class="message-container" id="m{$table.message_id}">
		...
		<a href="{$base_url}/message.php?id={$table.message_id}&amp;link={$link_id}&amp;r={$table.revision_id}">
			Message Detail{if $table.revision_id > 1} ({$table.revision_id} edits){elseif $table.revision_id == 1} ({$table.revision_id} edit){/if}
		</a> | 
		<a href="{$base_url}/postmsg.php?link={$link_id}&amp;quote={$table.message_id}" onclick="return quickpost_quote('l,{$link_id},{$table.message_id}@{$table.revision_id}');">Quote</a>
	</div>
			...
			<tr>
				...
				<td class="userpic">
					<div class="userpic-holder">
                        {if $table.sha1_sum != NULL}<a href="{$base_url}/imagemap.php?hash={$table.sha1_sum}"><img src="./templates/default/images/grey.gif" data-original="{$base_image_url}/t/{$table.sha1_sum}/{$table.filename}" width="{$table.thumb_width}" height="{$table.thumb_height}" /></a>{/if}
...
{/foreach}
    ...
    {while $k <= $page_count}
        {if $k == $current_page}{$k} 
            {if $k<$page_count}|{/if}
            {else}
                <a href="{$base_url}/linkme.php?l={$link_id}&amp;page={$k}">{$k}</a> 
                {if $k < $page_count}| {/if}
        {/if}
